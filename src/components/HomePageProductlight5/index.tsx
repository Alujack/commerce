import React from "react";
import { Button, Img, Heading, Text } from "./..";

interface Props {
  className?: string;
  subtitle?: string;
  title?: string;
  price?: string;
  image?: string;
  buyNow?: string;
}

export default function HomePageProductlight5({
  subtitle = "Extra Bass",
  title = "Sony Black Headphones",
  price = "$100",
  image,
  buyNow = "Buy Now",
  ...props
}: Props) {
  return (
    <div {...props}>
      <Img src="images/defaultNoData.png" alt="extra_bass_one" className="w-full md:h-auto object-cover rounded" />
      <div className="flex flex-col self-stretch items-start pb-3.5 gap-[13px]">
        <div className="flex flex-col items-start">
          <Text as="p" className="!text-blue_gray-500 !font-manrope">
            {subtitle}
          </Text>
          <Heading as="h1" className="!text-gray-900_07">
            {title}
          </Heading>
        </div>
        <Heading size="7xl" as="h2" className="!text-gray-900_07">
          {price}
        </Heading>
        <div className="flex self-stretch gap-3">
          <div className="flex justify-center w-[31%] p-3 border-blue_gray-100_01 border border-solid bg-white-A700 rounded">
            {!!image ? <Img src={image} alt="image" className="h-[20px] w-[21px]" /> : null}
          </div>
          <Button
            color="gray_900_04"
            shape="round"
            rightIcon={<Img src="images/defaultNoData.png" alt="arrow_right" />}
            className="gap-2 sm:px-5 font-manrope font-semibold min-w-[142px] !rounded"
          >
            {buyNow}
          </Button>
        </div>
      </div>
    </div>
  );
}
